...
CREATE POLICY "Drivers can view own payout requests" ON public.driver_payout_requests
  FOR SELECT USING ((auth.uid() = driver_id) OR (public.get_current_user_role() = 'admin'));

CREATE POLICY "Drivers can create payout requests" ON public.driver_payout_requests
  FOR INSERT WITH CHECK ((auth.uid() = driver_id) AND (public.get_current_user_role() = 'driver'));
...
CREATE POLICY "Users can view ratings where they're involved or admins" ON public.driver_passenger_ratings
  FOR SELECT USING ((driver_id = auth.uid()) OR (passenger_id = auth.uid()) OR (public.get_current_user_role() = 'admin'));
...
-- Trigger para criar profile ao cadastrar usuário (requer Auth ativo)
...
DO $$
DECLARE enum_exists BOOLEAN;
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM pg_type 
    WHERE typname = 'payout_status' AND typtype = 'e'
  ) INTO enum_exists;
  IF enum_exists THEN
